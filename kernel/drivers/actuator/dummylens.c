...
static GPT_CONFIG	g_GPTconfig;		// --- Interrupt Config ---
...
static int FM50AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	//	  TODO : make sure enable 2.8V D out
	hwPowerOn(MT6516_POWER_VCAM_A, VOL_2800,"FM50AF");
		

	//TODO : move the GPIO out of here!!
	//    Set to low to enable lens actuator
	//CMPDN - GPIO 18

    if(mt_set_gpio_mode(18,0)){FM50AFDB("[FM50AF] set gpio mode failed!! \n");}
    if(mt_set_gpio_dir(18,1)){FM50AFDB("[FM50AF] set gpio dir failed!! \n");}
    if(mt_set_gpio_out(18,0)){FM50AFDB("[FM50AF] set gpio failed!! \n");}

    //if(mt_set_gpio_mode(97,0)){FM50AFDB("[FM50AF] set gpio mode failed!! \n");}
    //if(mt_set_gpio_dir(97,1)){FM50AFDB("[FM50AF] set gpio dir failed!! \n");}
    //if(mt_set_gpio_out(97,0)){FM50AFDB("[FM50AF] set gpio failed!! \n");}


	// --- Config Interrupt ---
	g_GPTconfig.num = GPT2;
	g_GPTconfig.mode = GPT_REPEAT;
	g_GPTconfig.clkDiv = GPT_CLK_DIV_1;
	g_GPTconfig.u4Timeout = 32;
	
	GPT_Init(g_GPTconfig.num, FM50AF_ISR);

	if (GPT_Config(g_GPTconfig) == FALSE)
	{
        FM50AFDB("[FM50AF] ISR Config Fail\n");	
		return -EPERM;
	}

	GPT_Start(g_GPTconfig.num);		
...
static int FM50AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	if (g_s4FM50AF_Opened)
	{
	    ...
	    if(mt_set_gpio_out(18,1)){FM50AFDB("[FM50AF] set gpio failed!! \n");}

    	hwPowerDown(MT6516_POWER_VCAM_A,"FM50AF");

		GPT_Stop(g_GPTconfig.num);
	}
...
{
//    int retVal = 0;
//    retVal = hwPowerDown(MT6516_POWER_VCAM_A,"FM50AF");
...
{
/*
    if(TRUE != hwPowerOn(MT6516_POWER_VCAM_A, VOL_2800,"FM50AF"))
    {
