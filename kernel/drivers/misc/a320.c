...
/*----------------------------------------------------------------------------*/
#define AVA_TAG					"[A320] "
#define AVA_DEV_NAME		    "A320"
#define AVA_FUN(f)				printk(KERN_INFO AVA_TAG"%s\n", __FUNCTION__)
 ...
 * local functions
*******************************************************************************/
static int a320_attach_adapter(struct i2c_adapter *adapter);
static int a320_detach_client(struct i2c_client *client);
...
static int a320_suspend(struct i2c_client *client, pm_message_t msg);
static int a320_resume(struct i2c_client *client);
#endif
...
/*----------------------------------------------------------------------------*/
/*for correct i2c address, please see cust_ofn.c*/
static unsigned short normal_i2c[] = {0x00, I2C_CLIENT_END};
static unsigned short ignore = I2C_CLIENT_END;
static struct i2c_client_address_data a320_addr_data = {
...
static struct i2c_driver a320_i2c_driver = {
	.attach_adapter = a320_attach_adapter,
	.detach_client  = a320_detach_client,
#if !defined(CONFIG_HAS_EARLYSUSPEND)    	
	.suspend        = a320_suspend,
	.resume         = a320_resume,
#endif
	.driver = {
		.name = AVA_DEV_NAME,
...
int a320_setup_eint(struct a320_priv *obj, void (*eint_motion)(void))
{
    ...
    MT6516_EINT_Set_Sensitivity(CUST_EINT_OFN_NUM, CUST_EINT_OFN_SENSITIVE);
    MT6516_EINT_Set_Polarity(CUST_EINT_OFN_NUM, CUST_EINT_OFN_POLARITY);
    MT6516_EINT_Set_HW_Debounce(CUST_EINT_OFN_NUM, CUST_EINT_OFN_DEBOUNCE_CN);
    MT6516_EINT_Registration(CUST_EINT_OFN_NUM, CUST_EINT_OFN_DEBOUNCE_EN, CUST_EINT_OFN_POLARITY, eint_motion, 0);

    MT6516_EINTIRQUnmask(CUST_EINT_OFN_NUM);
    return 0;
}
/******************************************************************************
...
static int a320_reset_and_init(struct i2c_client* client)
{
    ...
    mt_set_gpio_out(GPIO_OFN_DWN_PIN, GPIO_OUT_ONE);
    MT6516_EINTIRQMask(CUST_EINT_OFN_NUM);
    a320_power(obj->hw, 0);
...
#endif
/*----------------------------------------------------------------------------*/
static int a320_detect(struct i2c_adapter* adapter, int address, int kind)
{
	...
	client->driver = &a320_i2c_driver;
	client->flags = 0;
	strncpy(client->name, AVA_DEV_NAME , I2C_NAME_SIZE);    
...
static int a320_attach_adapter(struct i2c_adapter *adapter)
{
 	if (adapter->id == 2)   // TODO: custom
		return i2c_probe(adapter, &a320_addr_data, a320_detect);
	return -1;	
}
/*----------------------------------------------------------------------------*/
static int a320_detach_client(struct i2c_client *client)
{
	...
	if (err){
		AVA_ERR("Client deregistration failed, client not detached.\n");
		return err;
	}
