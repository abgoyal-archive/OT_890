...
static int ipanic_block_read_single(struct ipanic_data *ctx, loff_t offset, void *buf)
{
	int rc, len;
...
#endif
	if (rc == -EUCLEAN) {
		printk(KERN_WARNING "ECC Check sum error corrected %d\n", offset);
		rc = 0;
	}
...
{
#if 0
	printk("%s: ctx:%p file_offset:%d file_length:%d\n", __func__, ctx, file_length, file_offset);
#endif
	while (file_length > 0) {
...
{
	int len;
	struct ipanic_oops *oops = ipanic_oops_copy();
	len = sprintf(page, "aee-ipanic Oops\n");
	if (oops) {
		len += sprintf(page + len, "Process\n%s\nBacktrace\n%s", oops->header.process_path,
			       oops->header.backtrace);
...
static void mtd_panic_notify_add(struct mtd_info *mtd)
{
	...
	rc = ipanic_block_read_single(ctx, 0, ctx->bounce);
	if (rc < 0) {
...
{
	struct ipanic_data *ctx = &drv_ctx;
	struct ipanic_oops *oops;

	if ((ctx->curr.magic != AEE_IPANIC_MAGIC) || (ctx->curr.version != AEE_IPANIC_PHDR_VERSION)) {
	  return NULL;
	}

	oops = kmalloc(sizeof(struct ipanic_oops), GFP_KERNEL);
	if (oops != NULL) {
		memset(oops, 0, sizeof(struct ipanic_oops));

		if (ipanic_block_read(ctx, ctx->curr.oops_header_offset, ctx->curr.oops_header_length, &oops->header) != 0) {
			...
			void *ptr)
{
	...
	oops_header_offset = ALIGN(console_offset + console_len,
				   ctx->mtd->writesize);
	...
	printk(KERN_EMERG "aee-ipanic: Panic dump sucessfully written to flash\n");
