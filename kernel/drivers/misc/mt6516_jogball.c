...
int jbd_setup_eint(struct jbd_device *jb, struct jbd_eint_hlr *hlr)
{
    ...
    MT6516_EINTIRQMask(EINT_JBD_UP_NUM);
    MT6516_EINTIRQMask(EINT_JBD_LF_NUM);
    MT6516_EINTIRQMask(EINT_JBD_RG_NUM);
    MT6516_EINTIRQMask(EINT_JBD_DN_NUM); 

    atomic_set(&jb->dir[JBD_DIR_UP].polarity, EINT_JBD_UP_POLARITY);
    jb->dir[JBD_DIR_UP].eint_no = EINT_JBD_UP_NUM;
    MT6516_EINT_Set_Sensitivity(EINT_JBD_UP_NUM, EINT_JBD_UP_SENSITIVE);
    MT6516_EINT_Set_Polarity(EINT_JBD_UP_NUM, EINT_JBD_UP_POLARITY);
    MT6516_EINT_Set_HW_Debounce(EINT_JBD_UP_NUM, EINT_JBD_UP_DEBOUNCE_CN);   /*the actual debounce time will be multiplied as 32*/
    MT6516_EINT_Registration(EINT_JBD_UP_NUM, EINT_JBD_UP_DEBOUNCE_EN, EINT_JBD_UP_POLARITY, hlr->eint_up, 0);

    atomic_set(&jb->dir[JBD_DIR_LF].polarity, EINT_JBD_LF_POLARITY);
    jb->dir[JBD_DIR_LF].eint_no = EINT_JBD_LF_NUM;
    MT6516_EINT_Set_Sensitivity(EINT_JBD_LF_NUM, EINT_JBD_UP_SENSITIVE);
    MT6516_EINT_Set_Polarity(EINT_JBD_LF_NUM, EINT_JBD_LF_POLARITY);
    MT6516_EINT_Set_HW_Debounce(EINT_JBD_LF_NUM, EINT_JBD_LF_DEBOUNCE_CN);   /*the actual debounce time will be multiplied as 32*/
    MT6516_EINT_Registration(EINT_JBD_LF_NUM, EINT_JBD_LF_DEBOUNCE_EN, EINT_JBD_LF_POLARITY, hlr->eint_lf, 0);

    atomic_set(&jb->dir[JBD_DIR_RG].polarity, EINT_JBD_RG_POLARITY);
    jb->dir[JBD_DIR_RG].eint_no = EINT_JBD_RG_NUM;
    MT6516_EINT_Set_Sensitivity(EINT_JBD_RG_NUM, EINT_JBD_RG_SENSITIVE);
    MT6516_EINT_Set_Polarity(EINT_JBD_RG_NUM, EINT_JBD_RG_POLARITY);
    MT6516_EINT_Set_HW_Debounce(EINT_JBD_RG_NUM, EINT_JBD_LF_DEBOUNCE_CN);   /*the actual debounce time will be multiplied as 32*/
    MT6516_EINT_Registration(EINT_JBD_RG_NUM, EINT_JBD_RG_DEBOUNCE_EN, EINT_JBD_RG_POLARITY, hlr->eint_rg, 0);

    atomic_set(&jb->dir[JBD_DIR_DN].polarity, EINT_JBD_DN_POLARITY);
    jb->dir[JBD_DIR_DN].eint_no = EINT_JBD_DN_NUM;
    MT6516_EINT_Set_Sensitivity(EINT_JBD_DN_NUM, EINT_JBD_DN_SENSITIVE);
    MT6516_EINT_Set_Polarity(EINT_JBD_DN_NUM, EINT_JBD_DN_POLARITY);
    MT6516_EINT_Set_HW_Debounce(EINT_JBD_DN_NUM, EINT_JBD_DN_DEBOUNCE_CN);   /*the actual debounce time will be multiplied as 32*/
    MT6516_EINT_Registration(EINT_JBD_DN_NUM, EINT_JBD_DN_DEBOUNCE_EN, EINT_JBD_DN_POLARITY, hlr->eint_dn, 0);

    MT6516_EINTIRQUnmask(EINT_JBD_UP_NUM);
    MT6516_EINTIRQUnmask(EINT_JBD_LF_NUM);
    MT6516_EINTIRQUnmask(EINT_JBD_RG_NUM);
    MT6516_EINTIRQUnmask(EINT_JBD_DN_NUM); 
    return 0;
}
/*----------------------------------------------------------------------------*/
...
static void jbd_eint_key(unsigned long data)
{
    ...
    for (idx = 0; idx < JBD_DIR_NUM; idx++) {
        ...
        if (atomic_read(&dir->polarity) == EINT_CON_LOWLEVEL)
            atomic_set(&dir->polarity, EINT_CON_HIGHLEVEL);
        else
            atomic_set(&dir->polarity, EINT_CON_LOWLEVEL);        
        MT6516_EINT_Set_Polarity(dir->eint_no, atomic_read(&dir->polarity));
        MT6516_EINTIRQUnmask(dir->eint_no);
        flag |= (1 << idx);
    }
