...
/* Phase lock register */
#define PHASE_LOCK                      ((volatile u32*)(base+0x70))
#define PHASE_STA                       ((volatile u32*)(base+0x74))

/* Masks for MSDC_CFG */
#define MSDC_CFG_MSDC                   (0x1 << 0)      /* R/W */
#define MSDC_CFG_SYNCEN					(0x1 << 1)		/* R/W */
...
#define MSDC_IOCON_EN_SPD_UP_RD         (0x1 << 6)      /* R/W. Only MSDC_CFG_SYNCEN on */
#define MSDC_IOCON_EN_SPD_UP_WR         (0x1 << 7)      /* R/W. Only MSDC_CFG_SYNCEN on */
#define MSDC_IOCON_PRCFG3               (0x3 << 8)      /* R/W */
#define MSDC_IOCON_CMDRE                (0x1 << 15)     /* R/W */
#define MSDC_IOCON_DSW                  (0x1 << 16)     /* R/W */
#define MSDC_IOCON_INTLH                (0x1 << 17)     /* R/W */
#define MSDC_IOCON_LSDIO_INT_DLY        (0x1 << 18)		/* R/W */
...
#define SDC_CMD_RSPTYP                  (0x7 << 9)      /* R/W */
...
/* Masks for SDC_STA */
#define SDC_STA_SDCBUSY                 (0x1 << 0)      /* RO */
#define SDC_STA_CMDBUSY                 (0x1 << 1)      /* RO */
#define SDC_STA_DATBUSY                 (0x1 << 2)      /* RO */
...
/* Masks for PHASE_LOCK */
#define PHASE_LOCK_LKEN                 (0x1 << 0)  /* R/W */
#define PHASE_LOCK_DLYCTL               (0x1f << 1) /* R/W */
#define PHASE_LOCK_INVCTL               (0x1 << 6)  /* R/W */

/* Masks for PHASE_STA */
#define PHASE_STA_CMD_LK_NEG            (0x1 << 0)  /* R */
#define PHASE_STA_CMD_PHA_NEG           (0x7 << 1)  /* R */
#define PHASE_STA_DAT0_LK_NEG           (0x1 << 8)  /* R */
#define PHASE_STA_DAT0_PHA_NEG          (0x7 << 9)  /* R */
...
#define MPLL_CLK_DIV_208M			(0x000E)
#define UPLL_CLK_DIV_48M			(0x000A)
...
typedef struct {
    ...
    u32 pad1:4;
    u32 dat0_lock_neg:1;
    u32 dat0_phase_neg:3;
    u32 pad2:20;
} phase_sta_reg;

struct mt3351_sd_regs {
    ...
    u32               reserved3[5];
    phase_lock_reg   *phase_lock;   /* base+0x70h */
    phase_sta_reg    *phase_sta;    /* base+0x74h */

};

struct mt3351_sd_host
{
    struct mt3351_sd_host_hw    *hw;
    struct mt3351_sd_regs       *regs;
