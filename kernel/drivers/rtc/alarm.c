...
static bool pwron_alm = false;
static struct rtc_time pwron_tm;
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct rtc_time rtc_new_rtc_time, now_time;
...
from_old_alarm_set:
		...
		if (alarm_type == ANDROID_ALARM_POWER_ON) {
			...
			new_alarm_time.tv_sec = (new_alarm_time.tv_sec / 60) * 60;
			if (new_alarm_time.tv_sec == 0) {
				pwron_alm = false;
				alm.enabled = 3;
			} else {
				new_alarm_time.tv_sec -= 30;
				pwron_alm = true;
				alm.enabled = 2;
			}
			rtc_time_to_tm(new_alarm_time.tv_sec, &pwron_tm);
			alm.time = pwron_tm;
			spin_unlock_irqrestore(&alarm_slock, flags);
	...
	case ANDROID_ALARM_SET_RTC:
		...
		rtc_read_time(alarm_rtc_dev, &now_time);
		...
		rtc_tm_to_time(&now_time, &tmp_time.tv_sec);
		tmp_time.tv_nsec = 0;
...
{
	struct rtc_time *tm = (struct rtc_time *)p;
	struct rtc_device *rtc = alarm_rtc_dev;

	if (!(rtc->irq_data & RTC_AF))
		return;
	ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_INT, "rtc alarm triggered\n");
	spin_lock(&alarm_slock);
	if (pwron_alm && tm &&
	    tm->tm_mon == pwron_tm.tm_mon && tm->tm_mday == pwron_tm.tm_mday &&
	    tm->tm_hour == pwron_tm.tm_hour && tm->tm_min == pwron_tm.tm_min &&
	    tm->tm_sec >= (30 - 2) && tm->tm_sec <= (30 + 2)) {
		pwron_alm = false;
	}
	spin_unlock(&alarm_slock);
...
int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (alarm_enabled & ANDROID_ALARM_WAKEUP_MASK) {
		...
		if (pwron_alm) {
			...
			rtc_tm_to_time(&pwron_tm, &pwron_time);
			if (rtc_alarm_time > pwron_time + 2)
				rtc_alarm_time = pwron_time;
		}
		spin_unlock_irqrestore(&alarm_slock, flags);
