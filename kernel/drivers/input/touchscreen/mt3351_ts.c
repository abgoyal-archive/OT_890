 ...
 * note
 ****************************************************************************
 * 1. Sensitivity for touch screen should be set to edge-sensitive. But
 *    in this driver it is assumed to be done by interrupt core, though not done
 *    yet. Interrupt core may provide interface to let drivers set the
 *    sensitivity in the future. In this case, this driver should set the
 *    sensitivity of the corresponding IRQ line itself.
 ****************************************************************************/
...
/******************************************************************************
 * FUNCTION
 *  kal_uint16 ts_read_adc(kal_uint16 pos)
 *
 * DESCRIPTION
 *  This function triggers sample process of touch screen and returns 
 *  sample data.
 *
 * PARAMETERS
 *  pos           [IN]         Determines which of x, y, z1, z2 is to be sampled
 *
 * RETURNS
...
/******************************************************************************
 * FUNCTION
 *  void MT3351_ts_tasklet(unsigned long unused)
 *
 * DESCRIPTION
 *  The tasklet function that sends events stored in buffer to event handlers.
 *
 * PARAMETERS
 *  unused         [IN]       not used
 *
...
/******************************************************************************
 * FUNCTION
 *  void MT3351_ts_timer_fn(unsigned long unused)
 *
 * DESCRIPTION
 *  The function registered with timer. Called when the expire time set in timer
 *  
 *
 * PARAMETERS
 *  unused      [IN]    used to pass arguments, not used in this driver
 *
 * RETURNS
 *  none
 *
...
/******************************************************************************
 * FUNCTION
 *  irqreturn_t MT3351_ts_interrupt_handler(int irq, void *dev_id, 
 *  struct pt_regs *regs)
 *
 * DESCRIPTION
 *  The interrupt handler of touch screen. Do nothing but simply calls
 *  MT3351_ts_timer_fn.
 *
 * PARAMETERS
 *  irq       [IN]  IRQ number. Useful when you want to print log messages.
 *  dev_id    [IN]  Client data. Same as that passed to request_irq.        
 *  regs      [IN]  Rarely used. Holds a snapshot of the processor's context.
 *
 * RETURNS
 *  Returns IRQ_HANDLED if the device needs attention, otherwise the value 
 *  should be IRQ_NONE.
 *
 * RETURN VALUE LIST
 *  IRQ_HANDLED      if the device needs attention
 *  IRQ_NONE         otherwise
 *****************************************************************************/
...
/******************************************************************************
 * FUNCTION
 *  void MT3351_ts_device_init(void)
 *
 * DESCRIPTION
 *  The module initialization function. 
 *  This function is called when the module is loaded into kernel.
 *
 * PARAMETERS
 *  none
 *
...
/******************************************************************************
 * FUNCTION
 *  void MT3351_ts_device_exit(void)
 *
 * DESCRIPTION
 *  The module destruction function. 
