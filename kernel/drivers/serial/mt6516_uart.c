...
/* linux/drivers/serial/mt6516_uart.c
 *
 * (C) Copyright 2008 
...
/*
    In MT6516, the UART power on  is controlled by APMCUSYS_PDN_CLR0
                        power off is controlled by APMCUSYS_PDN_SET0    
    Bit07: URAT0
    Bit08: UART1
    Bit09: UART2
    Bit29: UART3
...
#if defined(ENABLE_DMA_VFIFO)
/* NOTICE:
 * 1. uart0 with hw flow control only can use vport0.
 * 2. vport1~5 are shared by uart0~4.
 */
...
/*
    if (!left) {
        if (!timer_pending(&vfifo->timer)) {
            vfifo->timer.expires  = jiffies + 10;
            vfifo->timer.function = mt6516_uart_dma_vfifo_timeout;
            vfifo->timer.data     = (unsigned long)&uart->dma_rx;
            add_timer(&vfifo->timer);
        }
    } else */
    if (left) {
