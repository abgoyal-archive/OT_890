...
/// mark_add for wifi oc ctrl begin

void pmic_vwifi3v3_disable_oc_ctrl(kal_bool bool_value){

    pmic6326_reg[0x33] = bool_value << 1;    /// bit one

    mt6326_write_byte(0x33, pmic6326_reg[0x33]);

}

 

void pmic_vwifi2v8_disable_oc_ctrl(kal_bool bool_value){

    pmic6326_reg[0x36] = bool_value << 1;    /// bit one

    mt6326_write_byte(0x36, pmic6326_reg[0x36]);

}

void pmic_vBT_disable_oc_ctrl(kal_bool bool_value){

    pmic6326_reg[0x3C] = bool_value << 2;    /// bit two £¬ notice£¡£¡£¡

    mt6326_write_byte(0x3C, pmic6326_reg[0x3C]);

}
/// mark_add for wifi oc ctrl end
...
#if defined(CONFIG_MT6516_PHONE_BOARD) || defined(CONFIG_MT6516_GEMINI_BOARD) || defined(CONFIG_MT6516_OPPO_BOARD) || defined(CONFIG_MT6516_E1K_BOARD)
...
void pmic_driver_init(void)
{
    ...
    pmic_vwifi3v3_disable_oc_ctrl(1);

    pmic_vwifi2v8_disable_oc_ctrl(1);

    pmic_vBT_disable_oc_ctrl(1);
    /// mark_add for wifi oc ctrl end
