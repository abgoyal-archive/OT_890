...
/* linux/drivers/i2c/busses/i2c-mt6516.c
 *
 * (C) Copyright 2008 
 * MediaTek <www.mediatek.com>
 * Infinity Chen <infinity.chen@mediatek.com>
 *
 * mt6516 I2C Bus Controller
 *
 ...
 * 02. mt6516 doesn't support to probe client device (only send slave addr)?
 *     => Not required.
 * 03. mt6516 doesn't support 10-bit slave address?
 *     => TBC
...
/*
static int mt6516_i2c_control(struct i2c_adapter *adapter, unsigned int cmd, 
                              unsigned long arg)
{
	struct mt6516_i2c *i2c = i2c_get_adapdata(adapter);
	u32 len, sclk = i2c->sclk;

	switch (cmd) {
	case I2C_MDELAY:
		len = 2 * sclk * arg;
		I2C_SET_TRANS_DELAY(len);
		break;
	case I2C_UDELAY:
		len = 2 * (sclk * arg + 1000 - 1) / 1000;
		I2C_SET_TRANS_DELAY(len);
		break;
	}	
	return 0;
