	...
	struct pt_regs * regs)
{
	...
	mm = get_task_mm(p);
	if (mm != NULL) {
		vmsize = (mm->total_vm - mm->reserved_vm) << (PAGE_SHIFT-10);
		vmrss = get_mm_rss(mm);
	}
	printk("[MEMPROF][%05lu.%-6lu]:exec:%s(%d):VmSize:%lu:VmRss:%lu\n", secs, usec_rem, p->comm, p->pid, K(vmsize), K(vmrss));
	if (mm != NULL)
		mmput(mm);
	...
	printk("[MEMPROF][%05lu.%-6lu]:Used:%lu:Cached:%lu:Buffer:%lu:Free:%lu\n", 
