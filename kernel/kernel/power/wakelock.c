...
// Kelvin, modify wakelock flag
// static int wakelock_debug_mask = DEBUG_EXIT_SUSPEND | DEBUG_WAKEUP;
//extern int PM_WAKELOCK_FLAG;
int wakelock_debug_mask = DEBUG_EXIT_SUSPEND | DEBUG_WAKEUP;
// end of modify

module_param_named(wakelock_debug_mask, wakelock_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
static int print_lock_stat(char *buf, int len, struct wake_lock *lock)
{
	...
	if (lock->flags & WAKE_LOCK_ACTIVE) 
	{
		...
		if (!expired)
			...
			expire_count++; // this wakelock is expired
...
static void print_active_locks(int type)
{
	...
	list_for_each_entry(lock, &active_wake_locks[type], link) {
		if (lock->flags & WAKE_LOCK_AUTO_EXPIRE) 
		{
            ...
            // expired wake lock
			if (timeout <= 0)
				pr_info("[print_active_locks] wake lock %s, expired\n", lock->name);
            // non-expired wakelock
			else
				pr_info("[print_active_locks] active wake lock %s, time left %ld\n",
					lock->name, timeout);
		} 
        // non-autoexpired wakelock
		else
			pr_info("[print_active_locks] active wake lock %s\n", lock->name);
	}
}

/*
 * This function also judge the max wake lock time
 * 1. returns 0 if no wake locks of the specified type are active
 * 2. return non-zero if one or more wake locks are held
 * 3. returns -1 if one or more wake locks with no timeout are active or the 
	...
	list_for_each_entry_safe(lock, n, &active_wake_locks[type], link) {
		...
		else
		{
			if (wakelock_debug_mask & DEBUG_SUSPEND)	
				pr_info("[has_wake_lock_locked]: has infinite wakelock:(%s) \n",lock->name);
			return -1;
		}
	}
	if (wakelock_debug_mask & DEBUG_SUSPEND)		
		pr_info("[has_wake_lock_locked]: max_timeout:(%d) \n",max_timeout);
	return max_timeout;
}
...
long has_wake_lock(int type)
{
	...
	spin_lock_irqsave(&list_lock, irqflags);
	if (wakelock_debug_mask & DEBUG_SUSPEND)	
		pr_info("[has_wake_lock]: call has_wake_lock_locked\n");	
...
static void suspend(struct work_struct *work)
{
	...
	if (wakelock_debug_mask & DEBUG_EXIT_SUSPEND) 
	{
		struct timespec ts;
		struct rtc_time tm;
		getnstimeofday(&ts);
...
static void expire_wake_locks(unsigned long data)
{
    ...
    // calculate the max wakelock time or return 0 if there's no wakelock
...
//_name, _function, _expires, _data
