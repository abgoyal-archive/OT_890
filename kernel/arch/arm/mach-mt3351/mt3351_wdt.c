...
/*
 * General function call : read / write watchdog's register
 *
 * Function List :
 *					mt3351_wdt_SetTimeOutValue
 *					mt3351_wdt_SetResetLength
 *					mt3351_wdt_ModeSelection
 *					mt3351_wdt_Restart
 *					mt3351_wdt_SWTrigger
 *					mt3351_wdt_CheckStatus
 *					mt3351_wdt_SW_MCUPeripheralReset
 *					mt3351_wdt_SW_MMPeripheralReset
 */
...
/*
 * MT3351 WatchDogTimer's interrupt handler
 */
...
/*
static struct resource MT3351_wdt_resource[] = {
	[0] = {
		.start = 1,
		.end   = 3,
		.flags = 1,
...
static unsigned int s3c_device_iis_dmamask = 0xfffffffe;

struct platform_device MT3351_device_wdt = {
		.name				= "mt3351-wdt",
		.id					= 0,
		.num_resources		= ARRAY_SIZE(MT3351_wdt_resource),
		.resource			= MT3351_wdt_resource,
		.dev				= {
			.dma_mask = &s3c_device_iis_dmamask,
			.coherent_dma_mask = 0xfffffffe
		}
};

EXPORT_SYMBOL(MT3351_device_wdt);
*/
