...
/*
{
#define MT6516ECO_VERSION (0xF0001000)
    unsigned long u4RegVal = 0;

    u4RegVal = ioread32(MT6516ECO_VERSION);

    if( (0x8a00 != u4RegVal) && (0x8a01 != u4RegVal) )
    {
        IDPDB("Invalid MT6516 chip version \n");
        return -EIO;
    }

    if( 0x8a01 == u4RegVal )//E2
    {
        //Read current frame buffer
        u4RegVal = ioread32(MT6516IMGDMA0_CURR_FRAME);
        *a_pu4IRT1Buff = ((u4RegVal & (0x3<<30)) >> 30);
        u4RegVal = ioread32(MT6516IMGDMA1_VDOENC_WXCNT);
        *a_pu4VDODMABuff = (u4RegVal & 0x1);
    }

    if( 0x8a00 == u4RegVal )//E1
    {
        *a_pu4IRT1Buff = g_u4IRT0Buff;
        *a_pu4VDODMABuff = g_u4VdoBuff;
    }
}
*/
#if 0
...
/*
        if((NULL == g_stPRZLineBuff.pYBuff) || (NULL == g_stPRZLineBuff.pUBuff) || (NULL == g_stPRZLineBuff.pVBuff))
        {
...
static int MT6516IDP_ConfigHW(unsigned long a_pNodeCfg)
{
    ...
    int i4RetVal = 0;
/*
    if(copy_from_user(&NodeCfg, (struct stNodeCfg *)a_pNodeCfg , sizeof(stNodeCfg))){
        IDPDB("[MT6516_IDP] ConfigHW, Copy from user failed 1 \n");
        return -EFAULT;
    }    

    pNextNodeCfg = &NodeCfg;
*/
...
/*
    IDPDB("LCD REG:\n ********************\n");
    for(u4Index = 0 ; u4Index < 0x138 ; u4Index += 4){
        u4RegValue = ioread32(LCDSTA+u4Index);
        IDPDB("+0x%x 0x%x\n", u4Index,u4RegValue);
    }
    for(u4Index = 0x200 ; u4Index < 0x224 ; u4Index += 4){
        u4RegValue = ioread32(LCDSTA+u4Index);
        IDPDB("+0x%x 0x%x\n", u4Index,u4RegValue);
    }    
*/
...
MT6516PRZ_INT[0:2]
MT6516DRZ_STA[0]
MT6516DRZ_ACKINT[0]//Write only
MT6516IMGDMA0_STA[4,5,8,10,13,15]//IBW1,IBW2,IBR1,OVL,IRT1,IRT3
MT6516IMGDMA0_ACKINT[4,5,8,10,13,15]//write only
MT6516IMGDMA1_STA[0,1,2,3,12]//JPEG,VDOENCW,VDOENCR,VDODEC,IRT0
*/
...
{
/*
    iowrite32(MT6516IMGDMA0_SW_RSTB,0);
    mdelay(1);
    iowrite32(MT6516IMGDMA0_SW_RSTB,1);
*/
//
//    iowrite32((1<<7),MT6516IMGDMA1_JPEG_CON);
//    iowrite32((1<<5),MT6516IMGDMA0_IBW2_CON);
